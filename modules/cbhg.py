...
class CBHG(Module):
    """Encoder CBHD
        It consists of:
            ...
            - bidirectional GRU
    """
    def __init__(self, sample_size, conv_bank_max_filter_size=16, conv_projections_channel_size=(128, 128), num_highways=4):
        ...
        for k in range(1, conv_bank_max_filter_size + 1):
            self.conv1d_banks.append(BatchNormConv1d(sample_size, sample_size, kernel_size=k, stride=1,
                             padding=k//2, activation=self.relu))
        self.conv1d_banks = ModuleList(self.conv1d_banks)
        ...
        for (in_size, out_size, ac) in zip(out_features, conv_projections_channel_size, activations):
            self.conv1d_projections.append(BatchNormConv1d(in_size, out_size, kernel_size=3, stride=1,
                                    padding=1, activation=ac))
        self.conv1d_projections = ModuleList( self.conv1d_projections)

        # Highway layers
        self.pre_highway = Linear(conv_projections_channel_size[-1], sample_size, bias=False)
        self.highways = ModuleList(
            [Highway(sample_size, sample_size) for _ in range(num_highways)])

        # bi-directional GPU layer
        self.gru = GRU(
            sample_size, sample_size, 1, batch_first=True, bidirectional=True)
